...
export default function ProductSpaceView() {
  ...
  return <main className="min-h-screen bg-[#fff9f1] text-[#22271f]">
    <div className="px-3 py-5 sm:px-4 lg:px-5">
      ...
      <div className={`mt-5 grid gap-5 ${error ? "grid-cols-1" : "xl:grid-cols-[minmax(0,1fr)_420px]"}`}>
        ...
        <aside className={`${error ? "hidden" : "block"} self-start overflow-hidden rounded-2xl border border-[#ddd2c2] bg-white shadow-[0_18px_50px_rgba(70,52,31,.10)] xl:sticky xl:top-32`}>
          {active ? <><div className="flex items-start justify-between gap-3 p-5"><div><p className="text-xs text-[#798078]">Moodboard đang chọn</p><h2 className="mt-1 text-2xl font-black">{active.title ?? "Moodboard"} ✎</h2><p className="mt-1 text-xs text-[#72786f]">{ROOM_TYPE_LABELS[active.roomType ?? "other"] ?? "Không gian"} · {points.length} sản phẩm</p></div><button className="rounded-lg bg-[#79943b] px-4 py-2 text-xs font-black text-white" type="button">Chia sẻ ↗</button></div>
            ...
            <section className="p-5"><div className="flex items-center justify-between"><h3 className="text-lg font-black">Ghi chú</h3><span className="text-xs font-bold text-[#78913f]">Chỉnh sửa</span></div><p className="mt-2 text-sm leading-6 text-[#666d65]">{active.description || "Chưa có ghi chú cho Moodboard này."}</p><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-[#eef4df] px-3 py-1 text-[11px] font-bold text-[#667c38]">{ROOM_TYPE_LABELS[active.roomType ?? "other"] ?? "Không gian"}</span>{active.isFeatured ? <span className="rounded-full bg-[#fff0c9] px-3 py-1 text-[11px] font-bold text-[#9a7020]">Nổi bật</span> : null}</div></section>
    ...
    {createOpen && <div className="fixed inset-0 z-[80] grid place-items-center bg-[#253128]/50 p-4 backdrop-blur-sm" onMouseDown={() => setCreateOpen(false)}><form className="w-full max-w-lg rounded-[26px] bg-[#fffdf8] p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()} onSubmit={createMoodboard}><div className="flex items-start justify-between"><div><p className="font-accent text-lg text-[#78943c]">Gom cảm hứng của bạn ✦</p><h2 className="text-3xl">Tạo Moodboard mới</h2></div><button className="rounded-full px-3 py-2 text-xl" onClick={() => setCreateOpen(false)} type="button">×</button></div><div className="mt-5 space-y-3"><input className="w-full rounded-xl border border-[#ddd2c2] bg-white px-4 py-3" maxLength={MOODBOARD_TITLE_MAXLENGTH} name="title" placeholder="Tên Moodboard" required/><textarea className="min-h-24 w-full rounded-xl border border-[#ddd2c2] bg-white px-4 py-3" maxLength={MOODBOARD_DESCRIPTION_MAXLENGTH} name="description" placeholder="Mô tả ý tưởng của bạn..."/><select className="w-full rounded-xl border border-[#ddd2c2] bg-white px-4 py-3" name="roomType" required>{ROOM_TYPES.map((value) => <option key={value} value={value}>{ROOM_TYPE_LABELS[value]}</option>)}</select><label className="block rounded-xl border-2 border-dashed border-[#b9ca8e] bg-[#f6f9ed] p-5 text-center text-sm font-bold text-[#657a38]">Chọn ảnh căn phòng<input accept="image/jpeg,image/png,image/webp" className="mt-3 block w-full text-xs" name="image" required type="file"/></label><label className="flex items-center gap-2 text-sm"><input name="isPublic" type="checkbox" value="true"/> Công khai Moodboard với cộng đồng</label></div>{createError && <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{createError}</p>}<button className="mt-5 w-full rounded-xl bg-[#79943b] py-3 font-black text-white disabled:opacity-60" disabled={creating}>{creating ? "Đang tạo..." : "Tạo Moodboard"}</button></form></div>}
  </main>;
}
